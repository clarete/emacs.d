...
#+begin_src elisp
  ...
  ;; Prevent straight from installing ELPA copies of libraries that already ship
  ;; with Emacs (otherwise you can end up loading the built-in one first, and
  ;; later straight tries to load its copy, triggering "Feature ... is now
  ;; provided by ..." errors; e.g. `xref`).
  (dolist (pkg '(xref project eldoc flymake jsonrpc external-completion seq let-alist))
...
Setup fringe style.  Notice that this must always happen after setting
the theme, otherwise the fringe colors are set to the ~default~ of the
previously selected theme (in my case, the default theme).
...
#+begin_src elisp
  ...
  ;; Make window dividers match the theme instead of defaulting to white
  (set-face-attribute
   'window-divider nil
...
#+begin_src emacs-lisp
  (straight-use-package
   '(bazel :type git :host github :repo "bazelbuild/emacs-bazel-mode"))
#+end_src
...
Markdown anchor links are usually generated with a prefix followed by
a random hash.  And that hash is regenerated every time the org file
is exported to markdown, so anchors keep changing and they're not even
very readable to begin with.  Here's some small lil hack to replace
those with a slug generated off the text of the headline itself.

#+begin_src elisp
  (defun my/org-md-slug (s)
    "Generate a slug string off `s'."
    (let* ((s (downcase (replace-regexp-in-string "[^[:alnum:][:space:]-]" "" s))))
      (replace-regexp-in-string "[[:space:]]+" "-" s)))

  (with-eval-after-load 'ox
    (advice-add 'org-export-get-reference :around
                (lambda (orig datum info)
                  (if (and (org-export-derived-backend-p (plist-get info :back-end) 'md)
                           (eq (org-element-type datum) 'headline)
                           (org-element-property :raw-value datum))
                      (my/org-md-slug (org-element-property :raw-value datum))
...
Emacs can compile its lisp flavor into native code.  Since there could
be quite a few warnings, just suppress them as they can get noisy

#+begin_src elisp
  ;; native-comp-async-report-warnings-errors: Whether to report
  ;; warnings and errors from asynchronous native compilation.
